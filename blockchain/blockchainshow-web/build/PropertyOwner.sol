...
contract PropertyOwner {
    ...
    // transfer property from cur to next.
    // return the new size of trans_list of id.
    function transfer(uint256 id, address cur, address next) returns (uint finalSize) {
        uint len = size(id);
        // require:  transaction must maintain by Address[init], current owner is cur and next owner is not cur.
        // ZEROMEM msg.sender == init 而不是cur
        require(msg.sender == init && len > 0 && map[id][len - 1] == cur && cur != next);
        map[id].push(next);
        finalSize = len + 1;
    }
